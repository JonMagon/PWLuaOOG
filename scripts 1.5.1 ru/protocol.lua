...
function Received(opcode, length)
	...
	elseif opcode == 0x85 then
		...
		if wid ~= 100 then -- FromWID
			...
			--GetAddCashSNArg()
			--PrivateChat(wid, nick, "^FF0000тест");
			--PrivateChat(wid, nick, "^FF00FF&GМ& шепчет: Поздравляем, &" .. nick .. "&! У вас есть ровно минута, чтобы найти Вэй Сяобао и забрать свой приз!");
			--PrivateChat(wid, nick, "&GМ& шепчет: тест");
			-- if string.find(message, "d_gm", 1, true) ~= nil then
				-- PrivateChat(wid, nick, "GM права активированы. Теперь Вам доступна GM панель. Для использования отправьте d_panel")
				-- AlreadyGM[wid] = 1
			-- elseif string.find(message, "d_item", 1, true) ~= nil or string.find(message, "d_ban", 1, true) ~= nil or string.find(message, "d_restart", 1, true) ~= nil then
				-- PrivateChat(wid, nick, "Данная команда доступна только для администраторов.")
			-- elseif string.find(message, "d_panel", 1, true) ~= nil then
				-- AlreadyGM[wid] = 2
				-- PrivateChat(wid, nick, "Доступные команды:")
				-- PrivateChat(wid, nick, "d_item <id> — получение предмета в текущую сумку.")
				-- PrivateChat(wid, nick, "d_ban <id> <time> <reason> — блокировка персонажа.")
				-- PrivateChat(wid, nick, "d_restart — перезагрузка сервера.")
			-- elseif AlreadyGM[wid] == nil then
				-- PrivateChat(wid, nick, "Вам выданы GM права. Для активации отправьте в мировой чат команду d_gm")
				-- AlreadyGM[wid] = 0
			-- elseif AlreadyGM[wid] == 0 then
				-- PrivateChat(wid, nick, "Для активации GM прав отправьте в мировой чат команду d_gm")
			-- elseif AlreadyGM[wid] == 1 then
				-- PrivateChat(wid, nick, "Для активации GM панели отправьте в мировой чат команду d_panel")
